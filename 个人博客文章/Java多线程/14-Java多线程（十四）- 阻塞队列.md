阻塞队列是`Java5`线程新特征中的内容，`Java`定义了阻塞队列的接口`java.util.concurrent.BlockingQueue`，阻塞队列的概念是，一个指定长度的队列，如果队列满了，添加新元素的操作会被阻塞等待，直到有空位为止。同样，当队列为空时候，请求队列元素的操作同样会阻塞等待，直到有可用元素为止。
