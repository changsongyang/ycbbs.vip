...
在`Java5`中，专门提供了锁对象，利用锁可以方便的实现资源的封锁，用来控制对竞争资源并发访问的控制，这些内容主要集中在`java.util.concurrent.locks` 包下面，里面有三个重要的接口`Condition`、`Lock`、`ReadWriteLock`。
...
有关锁的介绍,`API`文档解说很多，看得很烦，还是看个例子再看文档比较容易理解。
```java
public class Test { 
...
`Java`中读写锁有个接口`java.util.concurrent.locks.ReadWriteLock`，也有具体的实现 `ReentrantReadWriteLock`，详细的API可以查看JavaAPI文档。
